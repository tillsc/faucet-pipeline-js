...
//   within source code (e.g. `["esnext", "jsx"]`)
// * `transpiler.browsers`, if `false`, suppresses automatic configuration via
//   Browserslist
...
function generateConfig({ extensions = [], // eslint-disable-next-line indent
		externals, format, compact, moduleName, transpiler }, { browsers }) {
	if(transpiler) {
		if(transpiler.browsers === false) {
			browsers = null;
		}
		...
		let settings = generateTranspilerConfig(transpiler, { browsers });
...
function generateTranspilerConfig({ features, jsx = {}, exclude }, { browsers }) {
	let settings = {};
	let plugins = [];
